...
/* harmony import */ var paho_mqtt__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(295);
...
var AMIMQTTClient = function () {
  function AMIMQTTClient(useSSL, host, port, path, username, password, serverName, options) {
    var _this = this;

    _classCallCheck(this, AMIMQTTClient);

    _defineProperty(this, "_L", {});

    _defineProperty(this, "_cnt", 0x01);
    ...
    this._client = new (paho_mqtt__WEBPACK_IMPORTED_MODULE_0___default().Client)(host, port, path, this._uuid);
    ...
    this._client.onConnected = function () {
      ...
      return _this._onConnected.apply(_this, args);
    };
    ...
    this._client.onConnectionLost = function () {
      ...
      return _this._onConnectionLost.apply(_this, args);
    };
    ...
    this._client.onMessageArrived = function () {
      ...
      return _this._onMessageArrived.apply(_this, args);
    };
    ...
    this._client.onMessageDelivered = function () {
      ...
      return _this._onMessageDelivered.apply(_this, args);
    };
    ...
    this._client.connect({
      ...
      onSuccess: function onSuccess() {
        ...
        return _this._onSuccess.apply(_this, args);
      },
      onFailure: function onFailure() {
        ...
        return _this._onFailure.apply(_this, args);
      }
    });
  }

  _createClass(AMIMQTTClient, [{
    key: "getUser",
    ...
    value: function subscribe(topic, options) {
      ...
      this._client.subscribe(topic, {
        ...
        onFailure: function onFailure(_, errorCode, errorMessage) {
    ...
    key: "send",
    value: function send(topic, payload, options) {
      options = options || {};
      var token = this._cnt++;
      var message = new (paho_mqtt__WEBPACK_IMPORTED_MODULE_0___default().Message)(payload);
      message.token = token;
      message.topic = topic;
    ...
    value: function execute(command, options) {
      ...
      options = options || {};
      var token = this._cnt++;
      var topic = "ami/".concat(this._serverName, "/command/").concat(options.converter || 'AMIXmlToJson.xsl');
      var message = new (paho_mqtt__WEBPACK_IMPORTED_MODULE_0___default().Message)("AMI-COMMAND<".concat(token, ",\"").concat(this._user, "\",\"").concat(this._uuid, "\">").concat(command));
      message.token = token;
      message.topic = topic;
      message.qos = options.qos || 0x000;
      message.retained = options.retained || false;
      ...
      var result = this._L[token] = $.Deferred();
      setTimeout(function () {
        if (token in _this2._L) {
          _this2._L[token].reject('timeout', token);

          delete _this2._L[token];
        }
    ...
    key: "_onConnected",
    value: function _onConnected(reconnect, uri) {
      var _this3 = this;

      if (reconnect) {
        console.log("onConnected: client `".concat(this._uuid, "` reconnected to server URI `").concat(uri, "`"));
      } else {
        console.log("onConnected: client `".concat(this._uuid, "` connected to server URI `").concat(uri, "`"));
      }

      this.subscribe(this._uuid).done(function () {
        if (_this3._userOnConnected) {
          _this3._userOnConnected(reconnect, uri);
        }
    ...
    key: "_onFailure",
    value: function _onFailure(responseObject) {
      if (responseObject.errorCode !== 0) {
        console.log("onFailure: client `".concat(this._uuid, "` rejected, cause: ").concat(responseObject.errorMessage));
      }
    ...
    value: function _onConnectionLost(responseObject) {
      if (responseObject.errorCode !== 0) {
        console.log("onConnectionLost: client `".concat(this._uuid, "` disconnected, cause: ").concat(responseObject.errorMessage));
      }
    ...
    value: function _onMessageArrived(message) {
      var topic = message.topic;
      var payload = message.payloadString;
      var m = payload.match(this._responseRegExp);

      if (topic === this._uuid && m) {
        var token = parseInt(m[1]);
        var data = m[2];
    ...
    value: function _onMessageDelivered(message) {
